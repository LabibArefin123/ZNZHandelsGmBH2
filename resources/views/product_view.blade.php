            ...
            <div class="product-view-hero">
                <div class="product-view-gallery">
                    <div class="product-view-thumbnails">
                        @foreach ($product->images ?? [$product->image] as $index => $image)
                ...
                <div class="product-view-info">
                    <span class="product-view-eyebrow">{{ strtoupper($product->category) }}</span>
                    <h1>{{ $product->name }}</h1>
                    ...
                    <div class="product-view-price">
                        ...
                        @if ($product->old_price)
                            <del>{{ $product->currency }} {{ number_format($product->old_price, 2) }}</del>
                        @endif
                    </div>
                    ...
                    @if ($product->sizes)
                        <div class="product-view-option">
                            <div class="product-view-option-head">
                                <span>Size</span>
                                <strong id="selectedSize">{{ $product->sizes[0] }}</strong>
                            </div>
                    ...
                    <div class="product-view-option product-view-quantity-option">
                        ...
                        <div class="product-view-quantity-row">
                            <div class="product-view-quantity">
                                <button type="button" id="quantityMinus"><i class="bi bi-dash"></i></button>
                                <input type="number" id="productQuantity" value="1" min="1"
                                    max="{{ max(1, $product->stock) }}">
                                <button type="button" id="quantityPlus"><i class="bi bi-plus"></i></button>
                            </div>
                    ...
                    <div class="product-view-benefits">
                        <div>
                            ...
                            <span><strong>Quality Checked</strong> Carefully selected products</span>
                        </div>
                        <div>
                            <i class="bi bi-box-seam"></i>
                            <span><strong>Reliable Delivery</strong> Secure product handling</span>
                        </div>
                        <div>
                            <i class="bi bi-headset"></i>
                            <span><strong>Customer First</strong> Dedicated support</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="product-view-details">
                ...
                <div class="product-view-tab-content" id="tab-details">
                    <div class="product-view-specifications">
                        <div><span>Brand</span><strong>{{ $product->brand ?? 'ZNZ Handels GmbH' }}</strong></div>
                        <div><span>SKU</span><strong>{{ $product->sku }}</strong></div>
                        <div><span>Material</span><strong>{{ $product->material ?? 'Premium Material' }}</strong></div>
                        <div><span>Origin</span><strong>{{ $product->origin ?? 'European Collection' }}</strong></div>
                        <div><span>Fit</span><strong>{{ $product->fit ?? 'Regular Fit' }}</strong></div>
                ...
                <div class="product-view-tab-content" id="tab-shipping">
                    <div class="product-view-shipping">
                        ...
                        <div><i class="bi bi-shield-check"></i>
            ...
            @if ($relatedProducts->count())
                <div class="product-view-related">
                    <div class="product-view-related-heading">
                        ...
                        <a href="{{ route('product') }}">View Collection <i class="bi bi-arrow-up-right"></i></a>
                    </div>
                        ...
                        @foreach ($relatedProducts as $related)
                            <article class="product-related-card">
                                ...
                                <div class="product-related-body">
                                    <small>{{ ucfirst($related->category) }}</small>
                                    <h3><a href="{{ route('product.view', $related) }}">{{ $related->name }}</a></h3>
                                    <div>
                                        <strong>{{ $related->currency }} {{ number_format($related->price, 2) }}</strong>
                                        <a href="{{ route('product.view', $related) }}"><i
